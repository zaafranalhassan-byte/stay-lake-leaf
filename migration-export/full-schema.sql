...
grant execute on function public.has_role(uuid, public.app_role) to authenticated;revoke execute on function public.has_role(uuid, public.app_role) from authenticated;
...
REVOKE EXECUTE ON FUNCTION public.grant_admin_for_lakeleaf_email() FROM PUBLIC, anon, authenticated;
...
CREATE POLICY "bookings authenticated read"
  ...
  TO authenticated
  USING (true);
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS cost numeric(10,2);-- Restrict SECURITY DEFINER helper functions.
...
GRANT EXECUTE ON FUNCTION public.get_reserved_ranges() TO anon, authenticated;-- ============ profiles ============
create table public.profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  full_name text,
  email text,
...
GRANT EXECUTE ON FUNCTION public.is_admin() TO authenticated;
REVOKE ALL ON FUNCTION public.get_reserved_ranges() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_reserved_ranges() TO anon, authenticated;CREATE POLICY "site-media public read" ON storage.objects FOR SELECT USING (bucket_id = 'site-media');
CREATE POLICY "site-media admin insert" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'site-media' AND public.is_admin());
CREATE POLICY "site-media admin update" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'site-media' AND public.is_admin());
CREATE POLICY "site-media admin delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'site-media' AND public.is_admin());